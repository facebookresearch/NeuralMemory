...
def distance_between(snapshots, query_configs, opts, obj_1=None, obj_2=None):
    ...
    """
    # WARNING currently can't differentiate if there are two blocks of the same type (e.g 2 holes).
    ...
    last_time_slice = min(
        max(list(r[obj_1].keys())), max(list(r[obj_2].keys()))
    )  # last common time slice
    distance = (
        (r[obj_1][last_time_slice]["x"] - r[obj_2][last_time_slice]["x"]) ** 2
...
def closest_farthest_object(
    snapshots, query_configs, opts, obj_memid=None, polarity=None
):
    """
...
def closest_farthest_from_loc(snapshots, query_configs, opts, pos=None, polarity=None):
    ...
    if not pos:
        ...
        polarity = choice((1, -1))
    pword = {-1: "closest", 1: "farthest"}[polarity]
    question_text = "which object is {} to ({})?".format(
        pword, " , ".join(map(str, pos))
    )
    e = snapshots["num_steps"] - 1
    dists = []
    for m, s in snapshots["reference_objects"].items():
...
def max_direction(
    snapshots, query_configs, opts, dtype=None, coord=None, direction=None, frame=None
):
    """
    ...
    if dtype == "cardinal":
        ...
        for s in snapshots["reference_objects"].values():
            try:
                if s[e]["node_type"] == nt:
                    l.append(
                        (s[e]["x"], s[e]["y"], s[e]["z"], s[e]["yaw"], s[e]["pitch"])
                    )
            except:
                e2 = list(s.keys())[-1]
        ...
        in_frame_dir = rotation.transform(
            rotation.DIRECTIONS[direction], yaw, pitch, inverted=True
        )
        dists = []
        b = {"x": x, "y": y, "z": z}
        for m, s in snapshots["reference_objects"].items():
            if s.get(e) is not None and check_physical(s[e]):
                if s[e]["ref_type"] != "location":
                    ...
                    if ip > 0:
                        dists.append((m, ip))
        question_text = "which object is the farthest away from {} {}?".format(

...
def save_files(examples, save_dir, encoder, opts, info, metadata_file_name):
    ...
    small_val_examples = examples[num_train_samples : num_train_samples + 100]
    ...
    if not opts.no_val:
        ...
        data_file_name = os.path.join(save_dir, "small_val.pth")
        print("saving to: {}".format(data_file_name))
        torch.save(small_val_examples, data_file_name)

        try:
            encoder.text_tokenizer.save(save_dir)
        except:
            pass
